...
import { Bot, CalendarDays, CheckSquare, ChevronDown, LayoutDashboard, LineChart, LogIn, Menu, Moon, Settings, Sparkles, Target, X, Sun, LogOut } from 'lucide-react';
...
export default function App() {
  ...
  const computedState = React.useMemo(() => ({
    tasks: PredictionEngine.calculateTaskPredictions(rawTasks),
    goals: PredictionEngine.calculateGoalSuccess(rawGoals, PredictionEngine.calculateTaskPredictions(rawTasks)),
    ...
    burnout: PredictionEngine.calculateBurnoutAndProductivity(PredictionEngine.calculateTaskPredictions(rawTasks)),
    engineStats: PredictionEngine.getEngineMetrics()
  }), [rawTasks, rawGoals, rawOpps, skills]);
  const handleAddTask = (task: Omit<Task, 'id' | 'completionProbability' | 'predictedCompletionDate' | 'delayRisk' | 'timeAllocationHours'>) => { setRawTasks(prev => [{ ...task, id: `t_${Date.now()}`, completionProbability: 50, predictedCompletionDate: '', delayRisk: 'low' }, ...prev]); showToast('Task added'); };
  ...
  const renderPage = () => {
    ...
    return <SettingsPlaceholder user={user} onLogout={logout} />;
  };
...
function SettingsPlaceholder({ user, onLogout }: { user: { email: string; name: string; picture?: string } | null; onLogout: () => Promise<void> }) { return <div className="mx-auto max-w-3xl"><h1 className="text-3xl font-semibold">Settings</h1><p className="mt-2 text-sm text-slate-400">Account, integrations and Nova preferences.</p><div className="mt-7 rounded-2xl border border-slate-800 bg-slate-900/70 p-6"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-4">{user?.picture ? <img src={user.picture} alt="" className="h-12 w-12 rounded-xl object-cover"/> : <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-violet-500/10 text-violet-300">{user?.name?.slice(0,2).toUpperCase()}</div>}<div><p className="font-semibold text-white">{user?.name}</p><p className="mt-1 text-sm text-slate-500">{user?.email}</p></div></div><button onClick={() => void onLogout()} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-500/20 px-4 py-2 text-sm font-medium text-red-300 hover:bg-red-500/10"><LogOut size={15}/> Sign out</button></div></div></div>; }
